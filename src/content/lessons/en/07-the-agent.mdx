...
That last move is the job. An agent stops when the work *looks* done, so without a check it can run, "looks done" is its only signal and you are the test suite. [Anthropic's guide](https://code.claude.com/docs/en/best-practices) now opens with exactly that: hand it something that returns pass or fail — a test, a build exit code, a linter, a screenshot comparison. If you cannot verify it, do not ship it.

All surfaces run the same engine, and your CLAUDE.md, settings and MCP servers are shared across the local ones.
...
| VS Code / JetBrains | inline diffs, plan review in the editor | the VS Code extension bundles its own CLI and does **not** put `claude` on your PATH |
...
Since 2026 the built-in starting mode on Pro, Max and Team plans is **auto** in terminal and VS Code sessions. Enterprise plans, Console API keys and every `claude -p` run still start in Manual. "Claude Code asks about everything by default" is out of date — which changes what you have to watch.
...
**Skills** are a folder with a `SKILL.md` in `.claude/skills/<name>/`. Only the description sits in context; the body loads when it is needed, which is why a skill beats another 40 lines of CLAUDE.md. Call it as `/name`, or let the agent pick it up when the description matches. It follows the open [Agent Skills](https://agentskills.io) standard, so skills are not Claude-only.
...
| **Devin Desktop** (ex-Windsurf) | IDE plus cloud agent | free, light quota | $20 | third name in two years: Codeium, Windsurf, Devin Desktop — tutorials go stale |
...
Six prompts covering most of the work you will actually do. Square brackets are yours to fill in. Nothing here is Claude-specific — they work in Codex, Cursor, Copilot and Kiro too.
...
<Callout type="tip" title="Where to actually do this">
Reading about an agent is not running one. [Lab 01](/en/labs/claude-code-first-session/) installs Claude Code and gets you through a first session. [Lab 02](/en/labs/plan-mode-build/) builds the QAIRU Event Sign-up page through plan mode, then breaks it on purpose and rewinds it. [Lab 03](/en/labs/claude-md-and-permissions/) makes the permission rules and CLAUDE.md from this module real on your own machine.
</Callout>
